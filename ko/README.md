...
이 README는 `ko/` 폴더 내용을 새 저장소 루트로 복사해 사용하는 독립 한국어판
기준으로 작성되어 있습니다.
...
- 커밋 황금률: 빌드되는 것, 받아오는 것, 만들어지는 것, 비밀인 것은 올리지
  않습니다. 예: `dist/`, `build/`; `node_modules/`, `vendor/`; cache와 log;
  `.env`, key, token.
...
원본 템플릿 저장소에서는 같은 구조의 영어 버전이 루트에 있습니다.

`ko/` 내용을 새 저장소 루트로 복사한 뒤에는 이 섹션을 프로젝트 상황에 맞게
유지하거나 제거하면 됩니다.
